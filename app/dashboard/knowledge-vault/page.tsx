...
function ago(value?:string){if(!value)return "";const diff=Date.now()-new Date(value).getTime();const m=Math.max(0,Math.floor(diff/60000));if(m<60)return `${m||1} min ago`;const h=Math.floor(m/60);if(h<24)return `${h} hr${h===1?"":"s"} ago`;const d=Math.floor(h/24);return `${d} day${d===1?"":"s"} ago`;}
...
type Analytics={reviewed:number;mastered:number;recallAccuracy:number;reviewDue:number;streak:number;byUniverse:Row[];byCategory:Row[];sessions:Session[]};
type Recent={id:string;title:string;category:string;universe:string;content_type?:string;progress?:{status?:string;last_seen_at?:string;last_result?:string}};
type Category={name:string;slug:string;universe:string;count:number};

const emptyAnalytics:Analytics={reviewed:0,mastered:0,recallAccuracy:0,reviewDue:0,streak:0,byUniverse:[],byCategory:[],sessions:[]};
...
export default function KnowledgeVaultDashboard(){
  ...
  useEffect(()=>{(async()=>{
    ...
    const [ar,lr,cr]=await Promise.all([
      ...
      fetch(`/api/knowledge-vault?action=categories`,{cache:"no-store"})
    ]);
    const [ad,ld,cd]=await Promise.all([ar.json(),lr.json(),cr.json()]);
    if(ad.ok)setA({...emptyAnalytics,...ad.analytics,sessions:ad.analytics?.sessions||[]});
    if(ld.ok){setRecent(ld.recent||[]);setGoal(ld.goal||null);}
  ...
  return <div className="dash-app kv-analytics-page">
    <header className="dash-header"><div className="dash-brand"><div className="dash-brand-mark">CF</div><div><b>Capital Forge</b><small>Master Finance. Build Your Future.</small></div></div><div className="dash-header-mid"><div className="dash-search"><span>⌕</span><input placeholder="Search topics, companies, trades, events or concepts..."/><kbd>⌘ K</kbd></div></div><div className="dash-header-right"><button className="dash-ai" onClick={()=>go("Advanced")}>✦ AI Assistant</button><div className="dash-profile"><div className="dash-avatar">DC</div><div><b>Deepak</b><small>Capital Forge</small></div><button className="dash-caret">⌄</button></div></div></header>
    <aside className="dash-sidebar"><nav className="dash-nav">{tabs.map(tab=><button key={tab} className={tab==="Dashboard"?"active":""} onClick={()=>tab==="Dashboard"?undefined:go(tab)}><span>{icons[tab]}</span>{tab}</button>)}</nav><div className="dash-upgrade"><h3>Knowledge Vault</h3><p>Your permanent finance memory system across 3,000 canonical learning objects.</p><button onClick={()=>go("Knowledge Vault")}>Continue Learning →</button></div><div className="dash-version">Capital Forge · Knowledge Analytics</div></aside>
    <main className="dash-workspace"><div className="kv-dash-wrap">
      ...
      <div className="kv-dash-title"><div><h1>Knowledge Vault Analytics</h1><p>Track your progress, recall strength, review schedule, and coverage across 3,000 finance knowledge objects.</p></div><label className="kv-range">▣ <select value={rangeDays} onChange={e=>setRangeDays(Number(e.target.value))}><option value={7}>Last 7 Days</option><option value={30}>Last 30 Days</option><option value={90}>Last 90 Days</option></select></label></div>

      <div className="kv-top-layout">
        <div className="kv-main-column">
          <div className="kv-dash-kpis">
            <Metric icon="▣" tone="blue" label="Reviewed" value={`${a.reviewed.toLocaleString()} / 3,000`} note={`${completion}% of the Vault covered`}/>
            <Metric icon="♜" tone="green" label="Mastered" value={a.mastered.toLocaleString()} note={`${asPercent(a.mastered,Math.max(1,a.reviewed))}% of reviewed objects`}/>
            <Metric icon="◎" tone="red" label="Recall Accuracy" value={`${a.recallAccuracy}%`} note="Based on rated Vault attempts"/>
            <Metric icon="♨" tone="orange" label="Current Streak" value={`${a.streak} day${a.streak===1?"":"s"}`} note="Meaningful Vault activity"/>
            <Metric icon="◷" tone="red" label="Review Due" value={a.reviewDue.toLocaleString()} note="Objects due now"/>
          </div>
          ...
          <div className="kv-lower-grid">
            <section className="kv-dash-card kv-heat-card"><div className="kv-dash-head"><h3>Universe Mastery Heatmap</h3><div className="kv-legend"><span><i className="green"/>High (≥80%)</span><span><i className="amber"/>Medium (50–79%)</span><span><i className="red"/>Low (&lt;50%)</span></div></div>{categories.length?<div className="kv-universe-heatmap">{categoriesByUniverse.map(group=><div className="kv-heat-group" key={group.universe}><h4><span>{universeIcons[group.universe]}</span>{group.universe}</h4>{group.rows.map(c=>{const p=categoryProgress.get(c.name);const accuracy=p?.attempts?p.accuracy:null;const cls=accuracy===null?"none":accuracy>=80?"high":accuracy>=50?"medium":"low";return <button key={c.slug} onClick={()=>window.location.assign(`/knowledge-vault/category/${c.slug}`)}><span>{c.name}</span><i className={cls}/></button>})}</div>)}</div>:<p className="kv-dash-empty">The 25-category mastery heatmap will populate automatically after the canonical CF3 import.</p>}</section>

            <section className="kv-dash-card kv-recent-card"><div className="kv-dash-head"><h3>Recent Vault Activity</h3><button onClick={()=>window.location.assign("/knowledge-vault/saved?kind=recent")}>View All →</button></div>{recent.length?<div className="kv-recent-list">{recent.slice(0,5).map((r,i)=><button key={r.id} onClick={()=>window.location.assign(`/knowledge-vault/object/${r.id}`)}><span className={`event e${i%5}`}>{r.progress?.status==="mastered"?"✓":r.progress?.last_result==="Again"?"↻":"▣"}</span><div><b>{r.progress?.status==="mastered"?"Mastered":r.progress?.last_result?`Reviewed: ${r.progress.last_result}`:"Reviewed"}: {r.title}</b><small>{r.universe} · {r.category}</small></div><em>{ago(r.progress?.last_seen_at)}</em></button>)}</div>:<p className="kv-dash-empty">Your latest learning, reviews, saves and mastery events will appear here.</p>}</section>
          </div>

          <div className="kv-continue"><span>✦</span><div><b>Keep going! You’re {completion}% through the Knowledge Vault.</b><p>Build consistency to master key concepts and strengthen your recall.</p></div><button onClick={()=>window.location.assign("/knowledge-vault/quick-scan")}>Continue Learning →</button></div>
        </div>

        <aside className="kv-right-column">
          <section className="kv-dash-card kv-activity-card"><div className="kv-dash-head"><h3>Learning Activity</h3><span>{rangeDays} days</span></div><div className="kv-donut-wrap"><div className="kv-donut" style={{background:donutStops}}><div><b>{sessionTotal}</b><small>Sessions</small></div></div><div className="kv-donut-legend">{modeRows.map((x,i)=><div key={x.name}><i className={`c${i}`}/><span>{x.name}</span><b>{x.count}</b><em>{sessionTotal?`(${Math.round(x.count/sessionTotal*100)}%)`:"(0%)"}</em></div>)}</div></div><div className="kv-activity-foot"><span>{minutes} min studied</span><span>{sessions.length} active sessions</span></div></section>
...
function TrendChart({points}:{points:{date:Date;value:number}[]}){
  if(points.length<2)return <div className="kv-trend-empty"><span>⌁</span><b>Recall trend will appear after scored sessions.</b><small>No sample values are fabricated.</small></div>;
