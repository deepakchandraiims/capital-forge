...
export default function NavRouteBridge() {
  ...
  useLayoutEffect(() => {
    // These two legacy controls are inserted before paint so they cannot create
    // a visible layout jump during route changes.
    if (pathname === "/practice" && !document.querySelector(".cf-qm-promo")) {
      ...
      if (hero?.parentElement) {
        ...
        card.querySelector("button")?.addEventListener("click", () => router.push("/practice/quick-math", { scroll: false }));
    ...
    if (pathname === "/dashboard" && !document.querySelector(".kv-injected-dashboard-switch")) {
      ...
      if (host) {
        ...
        vault.addEventListener("click", () => router.push("/dashboard/knowledge-vault", { scroll: false }));
