...
type Health = { keyStatus?: Record<string, boolean>; phase?: string; status?: string };
...
const storeKey = "capital-forge-practice-workstation-fixed-v3";
...
const questions: Question[] = [
  ...
  { id: "q6", title: "What are the most important checks in a three-statement model?", helper: "Think like a model reviewer before an investment committee.", category: "Accounting", subtopic: "3-Statement", difficulty: "Intermediate", qtype: "Accounting", minutes: 5, xp: 75, modelAnswer: "Balance sheet balances, cash flow reconciles, working capital rolls forward, debt and interest link correctly, taxes are logical, signs are consistent and hardcodes are limited." }
];

function pct(n: number, d: number) { return d ? Math.round((n / d) * 100) : 0; }
function normalizeTone(value: unknown): Tone { return value === "red" || value === "green" || value === "purple" || value === "black" || value === "amber" || value === "gray" || value === "blue" ? value : "blue"; }
function toJson(value: unknown) { return JSON.stringify(value, null, 2); }
function fiveNews(items: NewsItem[]) {
  const list = items.map((item, index) => ({ ...item, tone: normalizeTone(item.tone), imageUrl: item.imageUrl || fallbackImages[index % fallbackImages.length] }));
  for (let i = list.length; i < 5; i += 1) list.push({ ...fallbackNews[i % fallbackNews.length], id: `fallback-${i}` });
...
export default function CapitalForge() {
  ...
  const [apiResult, setApiResult] = useState<unknown>({ status: "ready", phase: "practice-ui" });
  ...
  const actualCorrect = store.attempts.filter((item) => item.correct).length;
  const actualIncorrect = store.attempts.filter((item) => !item.correct).length;
  const correct = store.attempts.length ? actualCorrect : 142;
  const incorrect = store.attempts.length ? actualIncorrect : 67;
  const skipped = store.attempts.length ? store.skipped : 23;
  const total = correct + incorrect + skipped;
  const accuracy = pct(correct, correct + incorrect);
  const connectedApis = ["supabaseConfigured", "newsConfigured", "marketConfigured", "backupMarketConfigured", "fundamentalsConfigured", "aiConfigured"].filter((key) => health?.keyStatus?.[key]).length;
  const visibleNews = useMemo(() => fiveNews(news), [news]);

  const visibleQuestions = useMemo(() => questions.filter((q) => {
    const haystack = `${q.title} ${q.helper} ${q.category} ${q.subtopic} ${q.qtype} ${q.difficulty}`.toLowerCase();
    const categoryOk = category === "All" || q.category === category || (category === "IB" && q.category === "Investment Banking") || (category === "PE" && q.category === "Private Equity");
    ...
    return categoryOk && difficultyOk && typeOk && timeOk && subtopicOk && queryOk && modeOk;
  ...
  async function refreshHealth() { try { const data = await getJson("/api/health"); setHealth(data); setApiResult(data); return data; } catch { return null; } }
  ...
  async function refreshQuote() { setBusy("quote"); try { const data = await getJson(`/api/market?symbol=${encodeURIComponent(symbol || "AAPL")}`); setQuote(data.quote || null); setApiResult(data); } catch {} finally { setBusy(""); } }
  async function testAI() { setBusy("ai"); try { const data = await getJson("/api/lab", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ module: "Practice AI", input: "Create one hard finance practice question." }) }); setApiResult(data); } catch (error) { setApiResult({ error: error instanceof Error ? error.message : "AI test failed" }); } finally { setBusy(""); } }
  ...
  function WeakAreas() { return <section className="practice-rail-card weak-card"><div className="rail-head"><div><h3>🎯 Weak Areas</h3><p>Focus on these topics</p></div><button onClick={() => setMode("Weak Areas")}>View All →</button></div>{[["Financial Modeling",14,"red"],["M&A",52,"red"],["Accounting",60,"amber"],["Valuation",62,"amber"]].map((row) => <button key={String(row[0])} onClick={() => { setMode("Weak Areas"); setCategory(String(row[0]) === "M&A" ? "PE" : String(row[0])); }}><span>{row[0]}</span><i><b className={String(row[2])} style={{ width: `${Number(row[1])}%` }}/></i><strong>{row[1]}%</strong></button>)}</section>; }
  function QuickPractice() { return <section className="practice-rail-card quick-practice"><div className="rail-head"><div><h3>⚡ Quick Practice</h3><p>Start with a quick session</p></div><button onClick={() => setMode("Custom Practice")}>⚙ Custom</button></div><div>{[["10","Questions","blue"],["20","Questions","green"],["Mixed","Difficulty","red"],["Previous","Mistakes","purple"]].map((row) => <button key={String(row[0])} className={String(row[2])} onClick={() => { setMode(String(row[0]) === "Previous" ? "Recently Practiced" : "All Questions"); start(questions[0]); }}><span>{row[0]}</span><small>{row[1]}</small></button>)}</div></section>; }
  ...
  function NewsCard({ item }: { item: NewsItem }) { return <article className="news-card"><div className="news-visual" style={{ backgroundImage: `url(${item.imageUrl || fallbackImages[0]})`, backgroundSize: "cover", backgroundPosition: "center" }} /><div className="news-meta"><span className={`pill ${item.tone}`}>{item.tag}</span><small>{item.time}</small></div><h3>{item.title}</h3><p>{item.summary}</p><div className="news-footer"><small>{item.source}</small><a href={item.url || "#"} target="_blank" rel="noreferrer">Read →</a></div></article>; }
