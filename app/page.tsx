...
type ApiResponse = { news?: NewsItem[]; quote?: Quote; keyStatus?: Record<string, boolean> } & Record<string, unknown>;
...
const storeKey = "capital-forge-practice-stable-v3";
...
const fallbackNews: NewsItem[] = [
  { id: "n1", tag: "Markets", tone: "green", title: "Equities rally on cooling inflation; tech leads gains", summary: "S&P 500 rises as investors weigh rates, earnings and positioning into quarter-end.", time: "2h ago", source: "Capital Forge", imageUrl: images[0] },
  { id: "n2", tag: "AI & Tech", tone: "purple", title: "AI capex cycle creates a new valuation debate", summary: "Turn this into a margin, DCF and terminal multiple practice question.", time: "3h ago", source: "Capital Forge", imageUrl: images[1] },
  { id: "n3", tag: "Strategy", tone: "blue", title: "PE firms stay selective as exits remain muted", summary: "Deal teams focus on resilient margins, cash conversion and debt capacity.", time: "4h ago", source: "Capital Forge", imageUrl: images[2] },
  { id: "n4", tag: "Business", tone: "red", title: "Renewables M&A accelerates across infra funds", summary: "Strategics and sponsors continue screening scale platforms in energy transition.", time: "5h ago", source: "Capital Forge", imageUrl: images[3] },
  { id: "n5", tag: "Global", tone: "blue", title: "Global markets mixed before central-bank decisions", summary: "Investors are watching rates, growth and risk appetite into the next policy cycle.", time: "6h ago", source: "Capital Forge", imageUrl: images[4] }
];
...
function fiveNews(items: NewsItem[]) {
  const out = items.map((item, index) => ({ ...item, tone: normalizeTone(item.tone), imageUrl: item.imageUrl || images[index % images.length] }));
...
export default function CapitalForge() {
  ...
  const visibleNews = fiveNews(news);
  ...
  const visibleQuestions = questions.filter((q) => {
    ...
    const searchOk = !query.trim() || haystack.includes(query.toLowerCase());
    ...
    return catOk && diffOk && typeOk && timeOk && subOk && searchOk && modeOk;
  }).slice(0, 4);
  ...
  async function refreshHealth() {
    ...
    setHealth(data as Health); setApiResult(data); return data;
  }
  ...
  function submit() {
    ...
    } else score = Math.max(35, Math.min(94, Math.round(clean.length / 3)));
  ...
  function NewsCard({ item }: { item: NewsItem }) { return <article className="news-card"><div className="news-visual" style={{ backgroundImage: `url(${item.imageUrl || images[0]})` }} /><div className="news-meta"><span className={`pill ${item.tone}`}>{item.tag}</span><small>{item.time}</small></div><h3>{item.title}</h3><p>{item.summary}</p><div className="news-footer"><small>{item.source}</small><a href={item.url || "#"} target="_blank" rel="noreferrer">Read →</a></div></article>; }
