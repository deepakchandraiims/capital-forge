    ...
    <main className="kv-workspace">
      <div className="kv-layout">
        <section className="kv-main">
          ...
          <section className="kv-hero"><div><h2>Real Finance Knowledge.<br/><span>Real-World Perspective.</span></h2><p>Study formulas, markets, famous trades, financial crises and more — all in one place.</p><button onClick={()=>router.push(heroRoute)}>{heroLabel}</button></div><blockquote>“The more context you have,<br/>the better decisions you make.”<i/></blockquote></section>

          <div className="kv-section-head"><h2>Explore by Learning Universe</h2><button onClick={()=>router.push("/knowledge-vault/learn")}>View All Categories →</button></div>
          ...
          ]).map((u)=>{const meta=universeCopy[u.name];return <button className={`kv-universe-card ${meta?.tone||"blue"}`} key={u.name} onClick={()=>router.push(`/knowledge-vault/universe/${u.slug}`)}><span className="kv-universe-icon">{meta?.icon}</span><h3>{u.name}</h3><small>{u.categories} categories</small><p>{meta?.copy}</p><b>Explore →</b></button>})}</div>
        ...
        <aside className="kv-rail">
          ...
          <section className="kv-rail-card"><div className="kv-card-head"><h3>Quick Learning</h3></div><div className="kv-quick-grid"><button onClick={()=>router.push("/knowledge-vault/quick-scan")}>⚡<span>Quick Scan</span></button><button onClick={()=>router.push("/knowledge-vault/learn")}>▤<span>Learn</span></button><button onClick={()=>router.push("/knowledge-vault/saved?kind=review")}>↻<span>Review</span></button><button onClick={()=>router.push("/knowledge-vault/quiz")}>✓<span>Quiz</span></button></div></section>
          <section className="kv-rail-card"><div className="kv-card-head"><h3>Recently Viewed</h3><button onClick={()=>router.push("/knowledge-vault/saved?kind=recent")}>View All →</button></div><div className="kv-recent">{data?.recent?.length?data.recent.slice(0,4).map((o)=><button key={o.id} onClick={()=>router.push(`/knowledge-vault/object/${o.id}`)}><span>◇</span><b>{o.title}<small>{o.category}</small></b><i>›</i></button>):<p>No objects viewed yet.</p>}</div></section>
