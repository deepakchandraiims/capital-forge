...
export default function KnowledgeVaultShell({ children, search, onSearch }: { children: ReactNode; search?: string; onSearch?: (value: string) => void }) {
  return <div className="kv-app">
    ...
    <aside className="kv-sidebar"><nav>{tabs.map((tab) => <button key={tab} className={tab === "Knowledge Vault" ? "active" : ""} onClick={() => go(tab)}><span>{icons[tab]}</span>{tab}</button>)}</nav><div className="kv-sidebar-card"><b>Knowledge Vault</b><p>Financial memory, context and pattern recognition across 25 categories.</p><button onClick={() => window.location.assign("/knowledge-vault/quick-scan")}>Start Quick Scan →</button></div><div className="kv-version">Capital Forge · CF3 Live<br/>3,000 canonical objects · progress tracked in real time.</div></aside>
    {children}
  </div>;
}
