...
function promptOf(item: CanonicalCase) {
  return pickText(item, ["prompt", "case_prompt", "question", "scenario", "context", "description", "title"]);
}

function answerOf(item: CanonicalCase) {
  return pickText(item, ["model_answer", "recommended_answer", "solution", "answer", "key_principle", "decision"]);
}

function label(value: unknown) {
  return String(value || "Case").replaceAll("_", " ").replace(/\b\w/g, (m) => m.toUpperCase());
}
...
export default function CasesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return cases.filter((item) => {
      ...
      const text = `${item.source_record_key || ""} ${item.title || ""} ${promptOf(item)} ${item.seniority || ""} ${(item.career_tracks || []).join(" ")}`.toLowerCase();
  ...
  return <div className="pm-shell">
    ...
    <main className="pm-main">
      ...
      <section className="pm-hero">
        <div><p className="pm-eyebrow">Canonical Cases · Supabase Live</p><h1>Investment Decision <span>Lab</span></h1><p>Train the part of finance interviews and investing that formulas cannot solve: judgment, trade-offs, downside framing, calibration and decision quality.</p><div className="pm-stats big"><div><small>Published Cases</small><b>{cases.length || "—"}</b></div><div><small>Decision Cases</small><b>{decisionCount || "—"}</b></div><div><small>Visible</small><b>{filtered.length}</b></div><div><small>Difficulty</small><b>1–10</b></div></div></div>
      ...
      <div className="pm-workspace" style={{ marginTop: 20 }}>
        <section className="pm-content">
          {selected && <section className="pm-panel">
            <div className="pm-panel-head"><div><span className="pm-tag blue">{selected.source_record_key || "CASE"}</span><h2 style={{ marginTop: 10 }}>{selected.title || label(selected.source_record_key)}</h2><p>{selected.seniority || "Investment judgment"} · Difficulty {selected.difficulty || "—"}</p></div><button className="pm-secondary" onClick={() => select(filtered[Math.floor(Math.random() * Math.max(filtered.length, 1))]?.id || selected.id)}>Random Case</button></div>
