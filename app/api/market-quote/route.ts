...
async function getQuote(requested:string){
  const symbol=requested.toUpperCase();
  const cached=quoteCache.get(symbol);
  if(cached&&cached.expiresAt>Date.now())return {...cached.body,cache:"memory-hit"};
  ...
  if(price==null)throw new Error("No market price");
  ...
  const body:Record<string,unknown>={
    ...
    quote:{
      symbol:requested,
      name:meta.shortName||meta.longName||requested,
  ...
  if(quoteCache.size>80){const first=quoteCache.keys().next().value as string|undefined;if(first)quoteCache.delete(first);}
  return {...body,cache:"memory-miss"};
}

export async function GET(request:Request){
  const {searchParams}=new URL(request.url);
  const batch=String(searchParams.get("symbols")||"").split(",").map(x=>x.trim()).filter(Boolean).slice(0,20);

  if(batch.length){
    const results=await Promise.all(batch.map(async symbol=>{
      try{return await getQuote(symbol);}
      catch(error){return {configured:false,symbol,error:error instanceof Error?error.message:"Quote unavailable"};}
    }));
    return NextResponse.json({configured:true,batch:true,results,generatedAt:new Date().toISOString()},{headers:{...HEADERS,"X-Capital-Forge-Batch":String(results.length)}});
  }

  const requested=String(searchParams.get("symbol")||"AAPL").trim();
  try{
    const body=await getQuote(requested);
    return NextResponse.json(body,{headers:{...HEADERS,"X-Capital-Forge-Cache":String(body.cache||"unknown")}});
  }catch(error){
    return NextResponse.json({configured:false,symbol:requested,error:error instanceof Error?error.message:"Quote unavailable"},{status:502,headers:{"Cache-Control":"public, max-age=5, s-maxage=10"}});
  }
}
