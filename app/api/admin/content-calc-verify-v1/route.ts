...
const MANIFEST_SHA256 = "ffae166d8296b371064ff9ea23e1ad901134fc70be40d20add559b81dcbe7bc2";
...
export async function POST(request: NextRequest) {
  ...
  if (sha256(text) !== MANIFEST_SHA256) {
    return NextResponse.json({ ok:false, error:"Verification manifest checksum mismatch. Use the exact 204-object file generated for this batch." }, { status:400 });
  }

  let manifest: any;
  try { manifest = JSON.parse(text); } catch { return NextResponse.json({ ok:false, error:"Invalid JSON manifest" }, { status:400 }); }
  if (manifest?.batch_id !== BATCH_ID || manifest?.calculation_objects !== 204 || !Array.isArray(manifest?.records) || manifest.records.length !== 204) {
    return NextResponse.json({ ok:false, error:"Manifest contract mismatch" }, { status:400 });
  }
  if (!manifest.records.every((r:any)=>r?.verified === true && typeof r?.source_record_key === "string" && typeof r?.evidence_sha256 === "string")) {
  ...
  if((rows||[]).length!==204) return NextResponse.json({ok:false,error:`Expected 204 calculation rows in staging; found ${(rows||[]).length}.`},{status:409});
  ...
  for(const rec of manifest.records){
    const row:any=byKey.get(rec.source_record_key);
    const {error:updateError}=await supabase.from("cf_content_staging").update({
      deterministic_status:"passed",
      deterministic_score:100,
      deterministic_result:{
        verifier:"capital-forge-calculation-audit-v1",
    ...
    if(updateError) return NextResponse.json({ok:false,stage:"update",key:rec.source_record_key,error:updateError.message,updatedBeforeFailure:updated},{status:500});
    updated++;
  }
  ...
  for(const r of after||[]){
    deterministic[String(r.deterministic_status)]=(deterministic[String(r.deterministic_status)]||0)+1;
    const k=`${r.content_type} / ${r.validation_status}`;
    structural[k]=(structural[k]||0)+1;
  }
  return NextResponse.json({ok:true,batchId:BATCH_ID,verifiedCalculationObjects:updated,deterministicStatusSummary:deterministic,structuralStatusSummary:structural,note:"Calculation verification applied only to the exact checksum-locked 204-object manifest. Nothing was published."});
}
